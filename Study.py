...
t2 = Task(1, description="Calculate the Net Subtotal by adding the Total Prices of all invoiced items.", formula="",
          expected_result_cell="F14", expected_operations = "", expected_operands="",
          result_type=RESULTTYPE.VALUE, max_total=4)
t3 = Task(1, description="Calculate the Gross Subtotal by adding 15% tax to the Net Subtotal.", formula="",
          expected_result_cell="F15", expected_operations = "", expected_operands="",
          result_type=RESULTTYPE.VALUE, max_total=5)
t4 = Task(1, description="Calculate the Quantity of Orange Juice.", formula="",
          expected_result_cell="A9", expected_operations = "", expected_operands="",
          result_type=RESULTTYPE.VALUE, max_total=5)
t5 = Task(1, description="Identify the maximum of all Discounts, and display it in cell J4.", formula="",
          expected_result_cell="J4", expected_operations = "", expected_operands="",
          result_type=RESULTTYPE.VALUE, max_total=4)
t6 = Task(1, description="Calculate the average of all Item Prices, round the result to the nearest whole number, and display it in cell J5.", formula="",
          expected_result_cell="J5", expected_operations = "", expected_operands="",
          result_type=RESULTTYPE.VALUE, max_total=5)
t7 = Task(1, description="Calculate the Packaging Fee by multiplying the Delivery Fee with the maximum of all Quantities and the average of all Discounts, and rounding the result to the nearest whole number.", formula="",
          expected_result_cell="F16", expected_operations = "", expected_operands="",
          result_type=RESULTTYPE.VALUE, max_total=9)
t8 = Task(1, description="Assuming there is a 'get one free' deal for Chocolate, calculate its Total Price.", formula="",
          expected_result_cell="F10", expected_operations = "", expected_operands="",
          result_type=RESULTTYPE.VALUE, max_total=7)
t9 = Task(1, description="Calculate the Total Price for each individual item by multiplying its Quantity and Item Price and applying the Discount.", formula="",
          expected_result_cell="F4:F12", expected_operations = "", expected_operands="",
          result_type=RESULTTYPE.LIST, max_total=7)
t10 = Task(1, description="Set the Delivery Fee to 10 € if the Gross Subtotal is less than 100 €, or 20 € otherwise.", formula="",
          expected_result_cell="F17", expected_operations = "", expected_operands="",
          result_type=RESULTTYPE.VALUE, max_total=8)
t11 = Task(1, description="Calculate the Invoice Amount by subtracting the Customer Credit from the Grand Total.", formula="",
          expected_result_cell="F26", expected_operations = "", expected_operands="",
          result_type=RESULTTYPE.VALUE, max_total=5)
t12 = Task(1, description="Calculate the Express Fee (in the Invoice) by multiplying the Gross Subtotal with the Express Rate (in the Inventory).", formula="",
          expected_result_cell="F18", expected_operations = "", expected_operands="",
          result_type=RESULTTYPE.VALUE, max_total=5)
t13 = Task(1, description="Calculate the Invoice Amount by subtracting the Customer Credit and the Holiday Discount (in the Inventory) from the Grand Total.", formula="",
          expected_result_cell="F26", expected_operations = "", expected_operands="",
          result_type=RESULTTYPE.VALUE, max_total=6)
t14 = Task(1, description="Determine the Item Price of Wine (in the Invoice) by using its Item No. to look up its Unit Price in the Inventory?", formula="",
          expected_result_cell="D6", expected_operations = "", expected_operands="",
          result_type=RESULTTYPE.VALUE, max_total=5)
t15 = Task(1, description="Calculate the Item Price of Laundry Detergent (in the Invoice) by adding the Low Stock Surcharge (in the Inventory) to its Unit Price if its Stock is lower than 5.", formula="",
          expected_result_cell="D12", expected_operations = "", expected_operands="",
          result_type=RESULTTYPE.VALUE, max_total=11)
t16 = Task(1, description="List the Item No's of all items whose Stock is lower than 5 next to'Low stock' in the Inventory?", formula="",
...
t17 = Task(1, description="List the Item No's of all invoiced items that are not included in the Inventory under 'Unknown items' in the Invoice.", formula="",
